...
const SCRIPT_DATA = [
  {
    id: 1,
    text: "ROSÉ Reminisces on Her BLACKPINK Audition, Shows Jimmy How to Play the APT. Drinking Game",
  },
  {
    id: 2,
    text: "ROSÉ Reminisces on Her BLACKPINK Audition, Shows Jimmy How to Play the APT. Drinking Game",
      ...
      <div className="h-[80px] bg-[#e7e2d9] border-b border-[#b4a69e]/30 flex items-center justify-between px-8 shrink-0 relative z-10">
        <div className="flex items-center gap-8">
          ...
          <h1 className="text-[length:var(--text-xl)] font-semibold text-black tracking-tight line-clamp-1 max-w-2xl">
            ROSÉ Reminisces on Her BLACKPINK Audition, Shows
            Jimmy How to Play the APT. Drinking Game
          </h1>
        </div>
      ...
      <div className="flex-1 flex gap-6 p-8 h-[calc(100vh-80px)]">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-6 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="flex items-center gap-3">
                ...
                <div className="bg-black/30 backdrop-blur-md px-3 py-2 rounded-full flex items-center gap-2 border border-[#b4a69e]/30">
                  <span className="text-[#f9f7f3] text-[length:var(--text-base)] font-medium">
                    0:03
                  </span>
                  <span className="text-[#f9f7f3]/50 text-[length:var(--text-base)] font-medium">
                    / 9:40
                  </span>
                </div>
        ...
        <div className="w-1/2 h-full bg-[#f9f7f3] rounded-2xl p-8 overflow-hidden flex flex-col">
          ...
          <div className="flex gap-3 mb-6 shrink-0">
            ...
            >
              {mode === "paragraph" && <CheckIcon />}한 문단씩
            </button>
            <button
...
function ScriptList({
  ...
  if (mode === "total") {
    const fullText = SCRIPT_DATA.map((s) => s.text).join(" ");
    return (
      <div className="flex flex-col gap-6 h-full">
        <div className="flex-1 overflow-y-auto">
          <p className="text-[length:var(--text-lg)] leading-relaxed font-medium text-black">
            {fullText}
          </p>
        </div>
        <div className="sticky bottom-0 pt-4 bg-[#f9f7f3]">
          <ShadowingPlayer />
        </div>
      </div>
    );
  ...
  if (mode === "paragraph") {
    // Split into 2 paragraphs for demo
    const paragraphs = [
      {
        id: 1,
        text: SCRIPT_DATA.slice(0, 3)
          .map((s) => s.text)
          .join(" "),
      },
      {
        id: 2,
        text: SCRIPT_DATA.slice(3)
          .map((s) => s.text)
          .join(" "),
      },
    ];
          ...
          <div key={p.id} className="flex flex-col gap-4">
            <p
              ...
              className={`
                                ...
                                ${expandedId === p.id ? "font-semibold text-black" : "font-medium text-[#a6a6a6] hover:text-[#808080]"}
                            `}
            >
              {p.text}
            </p>

            {expandedId === p.id && <ShadowingPlayer />}
          </div>
        ))}
        <div className="h-20" />
      </div>
    ...
    <div className="flex flex-col gap-6">
      {SCRIPT_DATA.map((line) => (
        <div key={line.id} className="flex flex-col gap-4">
          ...
          {expandedId === line.id && <ShadowingPlayer />}
        </div>
      ))}
      <div className="h-20" />
    </div>
